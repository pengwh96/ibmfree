...
display_welcome_message() {
    ...
    echo -e "${COLOR_MAGENTA}      欢迎使用 IBM-ws-nodejs 应用管理脚本 (${SCRIPT_VERSION})${COLOR_RESET}"
    echo -e "${COLOR_CYAN}===================================================================${COLOR_RESET}"
    echo ""
    echo -e "${COLOR_GREEN}此脚本将帮助您安装、配置和管理 IBM-ws-nodejs 应用。${COLOR_RESET}"
    echo -e "${COLOR_GREEN}应用将通过 PM2 进行管理，以确保其稳定运行和方便操作。${COLOR_RESET}"
    echo ""
    echo -e "${COLOR_YELLOW}作者 (Script Author): Joey (joeyblog.net)${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}核心功能实现 (Core Functionality By): eooce, qwer-search${COLOR_RESET}"
    echo -e "${COLOR_YELLOW}反馈TG (Feedback TG): https://t.me/+ft-zI76oovgwNmRh${COLOR_RESET}"
# --- 颜色定义 ---
COLOR_RED='\033[0;31m'
...
echo -e "${COLOR_GREEN}==================== IBM-ws-nodejs 配置生成脚本 ====================${COLOR_RESET}"
...
invoke_basic_configuration() {
    ...
    while true; do
        read -p "请输入您的域名（例如：yourdomain.freeIBM.com）: " domain_val
