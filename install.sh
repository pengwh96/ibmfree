...
configure_firewall() {
    ...
    while [[ "$confirmation" != "yes" && "$confirmation" != "no" ]]; do
        read -p "Are you sure you want to continue configuring the firewall to 'Allow All'? (Please enter 'yes' or 'no'): " confirmation
    ...
    if command -v iptables &>/dev/null; then
        echo -e "${COLOR_CYAN}iptables detected... Setting default policies to ACCEPT and flushing all rules...${COLOR_RESET}"
    ...
    echo -e "${COLOR_RED}Once again: Server security risk has been significantly increased. Please proceed with caution.${COLOR_RESET}"
    return 0
}
